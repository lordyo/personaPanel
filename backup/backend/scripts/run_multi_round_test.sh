#!/bin/bash
# Run the multi-round simulation tests
...
echo "Running multi-round simulation tests with model: $OPENAI_MODEL"
echo
...
if [ $? -eq 0 ]; then
    echo
    echo "✅ Multi-round simulations completed successfully!"
    echo "Results saved in multi_round_results directory"
else
    echo
    echo "❌ Multi-round simulations failed."
    exit 1
fi 